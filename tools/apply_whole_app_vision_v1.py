...
FOUNDATION_AND_DASHBOARD = r'''function nkAppSubjectMeta(name=activeSubject) {
  ...
  function nkAppSubjectIcon(name,size=20) {
    return navIcon(nkAppSubjectMeta(name).icon,size);
  }
  ...
  function nkWeekStrip() {
    ...
    for(let i=6;i>=0;i--){
      ...
      items.push(`<span class="nk-week-day ${active?'is-done':''} ${isToday?'is-today':''}" title="${isToday?'Today':d.toLocaleDateString()}"><i>${active?navIcon('check',11):''}</i><b>${names[d.getDay()]}</b></span>`);
    }
    return items.join('');
  }
  ...
  function dashboard() {
    ...
    return shell(`<div class="nk-app-v114 nk-home-v114">
      ...
      <section class="nk-streak-strip"><div class="nk-streak-copy"><span class="nk-streak-flame">${flameIcon(22)}</span><div><strong>${currentStreak()} day streak</strong><small>${currentStreak()?'Keep the chain going today.':'Your next answered question starts it.'}</small></div></div><div class="nk-week-strip">${nkWeekStrip()}</div></section>
      <section class="nk-focus-panel"><div class="nk-kicker">TODAY'S FOCUS</div><h2>${esc(focus)}</h2><p>${due?'Strengthen scheduled recall before adding new material.':wrong?'A second retrieval pass turns mistakes into memory.':'A compact mixed set is enough to build momentum.'}</p><div class="nk-focus-actions"><button class="nk-focus-primary" onclick="window.QB.continuePractice()">${navIcon('book',18)}<span>Continue Practice</span>${navIcon('chevron',17)}</button>${due?`<button onclick="window.QB.startLibrary('review')">Review ${fmtNum(due)}</button>`:`<button onclick="window.QB.startAllPractice()">Practice 20 Random Questions</button>`}<button onclick="window.QB.openTestBuilder()">Timed CBT</button></div></section>
...
HEADER = r'''function header(title = 'QBank'){
    return `<header class="nk-global-header-v114"><div class="nk-global-inner"><div class="nk-global-brand"><span>Q</span><strong>${esc(title)}</strong></div><div class="nk-global-subject is-${nkAppSubjectMeta().key}">${nkAppSubjectIcon(activeSubject,16)}<span>${esc(activeSubject)}</span></div></div></header>`;
  }
...
body:has(.nk-app-v114){background:var(--nk114-canvas)!important;color:var(--nk114-ink)}body:has(.nk-app-v114) .page{max-width:1040px!important;padding:22px 20px 104px!important}body:has(.nk-app-v114) .v102-topbar,body:has(.nk-app-v114) .topbar{display:none!important}.nk-app-v114{max-width:980px;margin:0 auto;color:var(--nk114-ink)}.nk-app-v114 button,.nk-global-header-v114 button,.nk-bottom-nav-v114 button{font:inherit}.nk-global-header-v114{height:58px;background:#fff;border-bottom:1px solid var(--nk114-line);position:sticky;top:0;z-index:80}.nk-global-inner{width:min(100% - 32px,980px);height:100%;margin:auto;display:flex;align-items:center;justify-content:space-between}.nk-global-brand{display:flex;align-items:center;gap:9px}.nk-global-brand>span{width:31px;height:31px;border-radius:9px;display:grid;place-items:center;background:var(--nk114-indigo);color:#fff;font-size:18px;font-weight:850}.nk-global-brand strong{font-size:14px}.nk-global-subject{display:flex;align-items:center;gap:7px;padding:6px 9px;border-radius:10px;background:#f3f4f8;color:#4b5575;font-size:11px;font-weight:750}.nk-global-subject.is-physiology{color:#ad4c4c;background:#fff1ef}.nk-global-subject.is-biochemistry{color:#972675;background:#f9edf6}.nk-global-subject.is-anatomy{color:#27758e;background:#ecf7fb}
...
.nk-streak-strip{min-height:68px;margin-top:14px;padding:10px 13px;background:#fff;border:1px solid var(--nk114-line);border-radius:13px;display:flex;align-items:center;justify-content:space-between;gap:14px}.nk-streak-copy{display:flex;align-items:center;gap:10px}.nk-streak-flame{width:36px;height:36px;border-radius:10px;background:#fff5de;display:grid;place-items:center}.nk-streak-copy strong,.nk-streak-copy small{display:block}.nk-streak-copy strong{font-size:13px}.nk-streak-copy small{font-size:10px;color:var(--nk114-muted);margin-top:2px}.nk-week-strip{display:flex;gap:5px}.nk-week-day{display:grid;gap:3px;justify-items:center}.nk-week-day i{width:22px;height:22px;border-radius:7px;background:#f0f2f6;display:grid;place-items:center;color:#fff}.nk-week-day b{font-size:9px;color:#8a90a3}.nk-week-day.is-done i{background:var(--nk114-blue)}.nk-week-day.is-today i{box-shadow:0 0 0 2px #cddcff}.nk-focus-panel{margin-top:16px;padding:22px;background:var(--nk114-indigo);color:#fff;border-radius:16px}.nk-focus-panel .nk-kicker{color:#bfc9f5}.nk-focus-panel h2{font-size:23px;line-height:1.17;letter-spacing:-.55px;margin:6px 0}.nk-focus-panel p{color:#c9cee5;margin:0;font-size:12px;line-height:1.45}.nk-focus-actions{display:grid;grid-template-columns:1.4fr .8fr .8fr;gap:8px;margin-top:18px}.nk-focus-actions button{min-height:48px;padding:8px 12px;border:1px solid rgba(255,255,255,.26);border-radius:11px;background:#3a376f;color:#fff;font-size:11px;font-weight:800}.nk-focus-actions .nk-focus-primary{background:#fff;color:var(--nk114-indigo);border-color:#fff;display:flex;align-items:center;justify-content:center;gap:7px}
