...
This helper exists only for recovery batch NKQ_BIOCHEM_COVERAGE_Q21_20260912.
It must be removed after the verified registry mutation is committed.
"""
...
from marrow_images import DATA, ROOT, render_region, sha, validate, write_json
...
def main() -> None:
    ...
    # Recreate the exact source-faithful render. The digest is a hard provenance gate.
    render_region("Biochemistry", PAGE, REGION, DPI, DATA / "images/originals")
    original_path = DATA / "images/originals" / f"{EXPECTED_SHA}.png"
    assert original_path.is_file(), "Expected Q21 source render was not produced"
    assert sha(original_path.read_bytes()) == EXPECTED_SHA, "Q21 render digest drift"

    production_path = DATA / "images/production" / f"{EXPECTED_SHA}.png"
    production_path.parent.mkdir(parents=True, exist_ok=True)
    if production_path.exists():
        assert production_path.read_bytes() == original_path.read_bytes(), "Production conflict"
    else:
    ...
    # Fail closed if this reference was already bound through a different asset.
    q21_bindings = [
        (asset, binding)
    ...
    # Do not duplicate an existing identical production asset under another ID.
