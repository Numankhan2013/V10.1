...
# This script is intentionally append-only. Never use a broad regex replacement
# against the 6MB monolithic index.html: a malformed match can delete product UI.
if marker in s:
    print('V10.3 overlay already present; leaving index.html unchanged')
    raise SystemExit(0)

css = r'''<style id="v103-review-explanation-css">
...
js = r'''<script id="v103-review-explanation-js">
(function(){
  'use strict';
  ...
  function reviewSessionFromTest(t){
    ...
    const ids=Array.isArray(t.questionIds)?t.questionIds.slice():[];
    return {id:'review_'+String(t.id),mode:'review',sourceTestId:String(t.id),title:'Review · '+String(t.title||'Completed Test'),questionIds:ids,index:0,answers:answers,submitted:Object.fromEntries(ids.map(id=>[id,Object.prototype.hasOwnProperty.call(answers,id)])),startedAt:t.createdAt||Date.now(),questionTimes:Object.assign({},t.questionTimes||{})};
  }
  function installReview(){
    ...
    reviewTest.__v103Wrapped=true;window.QB.reviewTest=reviewTest;
    const originalNext=window.QB.nextQ;
    if(typeof originalNext==='function'&&!originalNext.__v103Wrapped){
      function nextQ(){const state=window.QB.getState(),s=state&&state.activeSession;if(s&&s.mode==='review'){if(s.index<s.questionIds.length-1){s.index++;persist(state);location.hash='#review-test/'+encodeURIComponent(String(s.sourceTestId));}return false;}return originalNext.apply(this,arguments);}
    ...
    if(typeof originalPrev==='function'&&!originalPrev.__v103Wrapped){
      function prevQ(){const state=window.QB.getState(),s=state&&state.activeSession;if(s&&s.mode==='review'){if(s.index>0){s.index--;persist(state);location.hash='#review-test/'+encodeURIComponent(String(s.sourceTestId));}return false;}return originalPrev.apply(this,arguments);}
  ...
  function format(){document.querySelectorAll('.feedback-body,.explanation,.explanation-body').forEach(el=>{if(el.dataset.v103Formatted==='1')return;const txt=el.textContent||'';if(!txt.trim())return;const rich=richExplanation(txt);if(rich){el.innerHTML=rich;el.dataset.v103Formatted='1';}});}
...
print('V10.3 overlay appended safely')
