...
def transform(source: str) -> str:
    ...
    update_sw = """if ('serviceWorker' in navigator && location.hostname !== 'qbank.local') { window.addEventListener('load', async () => { try { const registration=await navigator.serviceWorker.register('./sw.js'); const offer=worker=>{if(!worker||document.querySelector('.nk-pwa-update'))return;const bar=document.createElement('div');bar.className='nk-pwa-update';bar.innerHTML='<span>A QBank update is ready.</span><button type=\"button\">Update</button>';bar.querySelector('button').onclick=()=>worker.postMessage('SKIP_WAITING');document.body.appendChild(bar);};if(registration.waiting)offer(registration.waiting);registration.addEventListener('updatefound',()=>{const worker=registration.installing;worker?.addEventListener('statechange',()=>{if(worker.state==='installed'&&navigator.serviceWorker.controller)offer(worker);});});let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!refreshing){refreshing=true;location.reload();}});}catch(error){console.warn('PWA update check unavailable',error);} }); }"""
    if simple_sw not in source:
        raise SystemExit("service worker registration anchor not found")
    source = source.replace(simple_sw, update_sw, 1)
