...
def transform(source: str) -> str:
    ...
    native_source = r'''    const url=seg=>`https://qbank.local/${subject}/pdf?page=${encodeURIComponent(seg.page)}&scale=3.5${seg.top!=null?`&top=${encodeURIComponent(seg.top)}`:''}${seg.bottom!=null?`&bottom=${encodeURIComponent(seg.bottom)}`:''}`;
