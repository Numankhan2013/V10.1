...
new = """            assert_sections(['General physiology','Nerve and muscle physiology','Gastrointestinal system','Cardiovascular system','Respiratory system','Central nervous system'])
            pnums=[int(x) for x in page.locator('.nk-topic-index').all_inner_texts()]
            if pnums!=list(range(1,34)): raise SystemExit(f'Marrow Physiology learner numbering is not contiguous 1-33: {pnums!r}')"""
...
new_summary = "biochemistry=543/26 physiology=753/33 physiology_plan=42 physiology_numbering=contiguous total=2115"
