...
for subject in ('Biochemistry','Physiology','Anatomy'):
    replacement = f'''            page.evaluate("window.QB.nkOpenSubjectLibrary('{subject}')");page.wait_for_timeout(120)'''
    for first_wait in (60,80,100,120):
        for second_wait in (60,80,100,120):
            old = f'''            page.evaluate("window.QB.nav('banks','{subject}')");page.wait_for_timeout({first_wait})\n            page.locator('button.nk-bank-card').filter(has_text='Marrow').click();page.wait_for_timeout({second_wait})'''
