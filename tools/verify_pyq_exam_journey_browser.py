        ...
        with sync_playwright() as playwright:
            ...
            for width, height in ((390, 844), (820, 1180)):
                ...
                for subject, topics, questions in (('Anatomy', 10, 410), ('Physiology', 9, 362), ('Biochemistry', 8, 346)):
                    open_builder()
                    choose_prep(subject)
                    expect(page.locator('.nk-cbt-pyq-action span')).to_have_text(f'{topics} eligible topics · {questions} questions')
                    page.get_by_role('button', name='Only verified PYQ topics').click()
                ...
                expect(page.locator('.nk-cbt-pyq-action span')).to_have_text('0 eligible topics · 0 questions')
                page.get_by_role('button', name='Only verified PYQ topics').click()
                expect(page.locator('#nk-cbt-footer-count')).to_have_text(before_empty)
                expect(page.locator('#nk-cbt-pyq-status')).to_contain_text('Your draft is unchanged')

                open_builder()
                page.get_by_role('button', name='Continue to topics').click()
                expect(page.locator('.nk-cbt-pyq-action span')).to_have_text('27 eligible topics · 1,118 questions')
                page.get_by_role('button', name='Only verified PYQ topics').click()
