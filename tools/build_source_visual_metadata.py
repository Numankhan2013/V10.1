...
def clean_question_text(text):
    s=str(text or '').replace('{{caption_text}}','')
...
def crop_box(page,bbox):
    """Tighten a PDF image resource to the meaningful figure content."""
    ...
    try:
        ...
        ix,iy=int(w*.06),int(h*.06); minx,miny=w,h; maxx=maxy=-1
        for yy in range(iy,max(iy,h-iy)):
            row=yy*w*3
            for xx in range(ix,max(ix,w-ix)):
                k=row+xx*3; rr,gg,bb=raw[k],raw[k+1],raw[k+2]
                # Ignore white/pale canvas while retaining dark labels, beige
                # anatomy, coloured regions and other substantive figure ink.
                if min(rr,gg,bb)<235 or max(rr,gg,bb)-min(rr,gg,bb)>20:
                    minx=min(minx,xx); maxx=max(maxx,xx); miny=min(miny,yy); maxy=max(maxy,yy)
        if maxx>=0 and maxy>=0:
            pad=max(7,int(min(w,h)*.035)); x0=max(ix,minx-pad); x1=min(w-ix-1,maxx+pad); y0=max(iy,miny-pad); y1=min(h-iy-1,maxy+pad)
            inv=1/scale; tight=fitz.Rect(r.x0+x0*inv,r.y0+y0*inv,r.x0+(x1+1)*inv,r.y0+(y1+1)*inv)
            if tight.width>=r.width*.12 and tight.height>=r.height*.12:r=tight
...
def page_question_starts(page):
    ...
    for b in page.get_text('blocks'):
        ...
        lines=max(1,text.count('\n')+1); line_h=max(8.0,(float(b[3])-float(b[1]))/lines)
...
def image_rect_candidates(page):
    """Prefer actual embedded PDF image resources; fall back to exposed blocks."""
    out=[];seen=set()
    for im in page.get_images(full=True):
        ...
        for r in rects:
            ...
            if r.get_area()>100 and key not in seen:seen.add(key);out.append(r)
    if out:return out
    for b in page.get_text('dict').get('blocks',[]):
        if b.get('type')==1 and b.get('bbox'):
            r=fitz.Rect(b['bbox'])
            if r.get_area()>100:out.append(r)
    return out
...
def main():
    ...
    for subject,(pdf_name,data_name) in SUBJECTS.items():
        ...
        if not qs:print('no questions for',subject,file=sys.stderr);sys.exit(1)
        ends=question_end_pages(qs,subject);starts_by_page={}
        ...
        for pno in sorted(active_by_page):
            ...
            for r in blocks:
                ...
                if crop:valid.append((owner,r.get_area(),{'type':'source-pdf','source':pdf_name,'page':pno,'crop':crop,'fit':'contain','scale':4.0}))
            if valid:
        ...
        for q in qs:
            question_text=q.get('_display_question','');cue=bool(VISUAL_CUE.search(question_text))
            if cue:cue_total+=1
            vals=sorted(assigned.get(q['id'],[]),key=lambda z:z[0],reverse=True)
            if not vals:continue
            if cue:cue_mapped+=1
            visuals=[v for _,v in vals];entries.append({'match':question_text,'visual':visuals[0],'visuals':visuals})
        if not entries:print(subject,'NO VALID QUESTION VISUALS',file=sys.stderr);sys.exit(1)
        allmeta[subject]=entries
        report[subject]={'questions':len(qs),'mappedQuestions':len(entries),'cueQuestions':cue_total,'cueMapped':cue_mapped,'pagesWithVisuals':pages_with_visuals,'imageBlocksAccepted':image_blocks}
    ...
    for subject,r in report.items():
        if r['mappedQuestions']<10:print('mapping floor failed for',subject,r,file=sys.stderr);sys.exit(1)
    OUT.write_text('/* Generated V11 source visual metadata. Exact QBank-indexed PDF image blocks; no fuzzy assignment. */\nwindow.SOURCE_VISUALS='+json.dumps(allmeta,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf-8')
