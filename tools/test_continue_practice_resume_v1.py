...
state.activeSession.index=1;state.activeSession.answers={q21:2};state.activeSession.submitted={q21:true};assert.equal(nkPausePractice(),true);
assert.equal(startSession(['q24','q25','q26'],'practice','Topic Three','normal'),true,'third chapter starts without replacing either saved session');
const thirdId=state.activeSession.id;
assert.equal(startSession(['q24','q25','q26'],'practice','Topic Three','normal'),true,'rapid duplicate start is idempotent');
assert.equal(state.activeSession.id,thirdId);
state.activeSession.index=2;state.activeSession.answers={q24:3};state.activeSession.submitted={q24:true};state.activeSession.pendingRating={q24:{rating:3}};
assert.equal(nkPausePractice(),true);
const thirdPaused=JSON.stringify(state.normalPracticeCheckpoints.find(cp=>cp.sessionId===thirdId));
assert.equal(nkPausePractice(),true,'double Pause is idempotent');
assert.equal(JSON.stringify(state.normalPracticeCheckpoints.find(cp=>cp.sessionId===thirdId)),thirdPaused);
assert.equal(nkPracticeCheckpoints(false).length,3);
...
const secondSaved=state.normalPracticeCheckpoints.find(cp=>cp.context.topicId==='t2');
...
assert.equal(JSON.stringify(state.normalPracticeCheckpoints.find(cp=>cp.sessionId===thirdId)),thirdPaused,'completing A must leave C intact');
let chooserHtml='';global.document.body={insertAdjacentHTML:(where,html)=>{chooserHtml=html;}};
window.QB.continuePractice();
assert(chooserHtml.includes('Paused Practice')&&chooserHtml.includes('Topic Two')&&chooserHtml.includes('Topic Three'),'Continue Practice must show the two remaining chapter choices');
assert.equal(nkResumePracticeById(thirdId),true);
assert.equal(state.activeSession.id,thirdId);
assert.deepEqual(state.activeSession.questionIds,['q24','q25','q26']);
assert.equal(state.activeSession.index,2);
assert.equal(state.activeSession.answers.q24,3);
assert.equal(state.activeSession.pendingRating.q24.rating,3);
assert.equal(nkPausePractice(),true);
assert.equal(nkDiscardNormalPractice(secondSaved.sessionId),true);
