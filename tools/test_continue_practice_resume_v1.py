...
assert.equal(state.fsrsReviewEligible.q5.reason,'skipped');assert.equal(state.activeSession.id,'same-session');
...
assert.equal(state.activeSession.questionIds.includes('q1'),false);assert.equal(Boolean(state.activeSession.submitted.q5),false);
assert.equal(state.activeSession.questionIds[state.activeSession.index],'q5');assert.equal(oldContinueCalls,0);
...
console.log('CONTINUE_PRACTICE_BEHAVIOR_OK remaining=16 answered_excluded=4 completed_next=t2');
'''
...
def main() -> None:
    ...
    print("CONTINUE_PRACTICE_CONTRACT_OK durable_pause=true session_submit=true canonical_next_topic=true")
