/* NK_CUSTOM_STUDY_MODULES_V1_START */
  ...
  function nkModuleBuilderPool(){
    ...
    const scopeSummary=`<div class="nk-module-scope-summary"><div><strong>Study scope · ${fmtNum(subjectCount)} subject${subjectCount===1?'':'s'}, ${fmtNum(scopes.length)} bank${scopes.length===1?'':'s'}</strong><p>${esc(scopes.join(' · '))}</p><small>${fmtNum(available)} questions match the selected topics and question type</small></div><div class="nk-module-scope-actions"><button onclick="window.QB.nkModuleBuilderStep(1)">Change banks</button><button onclick="window.QB.nkModuleBuilderStep(2)">Change topics</button></div></div>`;
    const types=[['all','All questions','Everything in the selected topics'],['unattempted','Unattempted','Questions you have not answered'],['wrong','Wrong','Questions missed before'],['bookmarked','Bookmarked','Questions you saved'],['mixed','Mixed','Prioritises wrong and unattempted']];
  ...
  function studyModuleBuilderPage(){
    if(!studyModuleDraft)studyModuleDraft=nkNewStudyModuleDraft();
    const step=studyModuleDraft.step||1,titles=['Choose question banks','Choose topics','Choose questions','Name and create'];
    ...
    const description=step===2?'Search or scroll, then tap the topics you want in this module.':step===3?'Your topics define the scope. Choose question type and count.':'Build a focused set that stays stable while you work through it.';
  ...
  function nkCreateStudyModule(startNow){
    if(!studyModuleDraft)return;const available=nkQuestionsForModuleDraft().length;if(!available){showToast('No eligible questions are available. Change your topics or question type.','bad');return;}
    ...
    if(!questionIds.length){showToast('No eligible questions are available. Change your topics or question type.','bad');return;}
