...
# Remove obsolete review wrapper layers; this build script is the single owner of the fix.
...
# Older Android WebViews: avoid Object.fromEntries in review-session construction.
s = s.replace(
...
# Replace reviewTest() by a staged, device-friendly opener.
...
if start >= 0 and end > start:
    replacement = '''function reviewTest(testId){
      ...
      try{
        ...
        stage='save'; saveState();
        stage='route'; route={page:'review-test',id:String(t.id)};
        stage='render'; render();
        stage='history';
...
# Remove the permanent navigator sidebars from Practice and Exam.
s = re.sub(r'<aside class="card navigator">.*?</aside>', '', s, flags=re.S)
s = re.sub(r'\n\s*const navItems=s\.questionIds\.map\(\(id,i\)=>\{.*?\}\)\.join\(\'\'\);', '', s, count=2)

# Navigator is now a single direct-access grid button: no More -> menu -> navigator chain.
...
s = s.replace('onclick="window.QB.notify()">${navIcon(\'more\')}', 'onclick="window.QB.openQuestionNavigator()">${navIcon(\'grid\')}', 3)

# Review gets the same direct navigator control.
old_review = '<section class="card question-card"><div class="crumb">${esc(q.chapter)}</div>'
new_review = '<section class="card question-card"><div class="q-head"><div class="q-number">Question ${q.questionNumber} of ${s.questionIds.length}</div><div class="q-actions"><button class="icon-btn" aria-label="Question Navigator" onclick="window.QB.openQuestionNavigator()">${navIcon(\'grid\')}</button></div></div><div class="crumb">${esc(q.chapter)}</div>'
s = s.replace(old_review, new_review, 1)

# Direct question navigator. In timed exams, keep the Submit Test action inside it.
block_marker = '  function endSession() { state.activeSession=null; saveState(); navigate(\'dashboard\'); }'
block = r'''  function closeQuestionNavigator(){ document.getElementById('qb-question-navigator')?.remove(); }
