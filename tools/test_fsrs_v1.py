...
assert(!practiceActionBar().includes('nk-fsrs-rating'),'no recall dock before answer submission');assert.equal(typeof nkFsrsSaveSettings,'function');assert(nkFsrsSettingsMarkup.toString().includes('Save changes')&&nkFsrsSettingsMarkup.toString().includes('nk-fsrs-customization'));
