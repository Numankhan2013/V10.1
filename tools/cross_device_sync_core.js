 ...
 */
  ...
  const NK_SYNC_META_LKG_KEY='qbank_sync_lkg_v2';
  ...
  function nkQueueEnvelope(envelope){
    ...
    nkSyncMeta.localHashes[key]=signature;envelope.updatedAt=Math.max(Date.now(),Number(envelope.updatedAt||0));nkSyncMeta.outbox[key]=envelope;nkCloudRevision++;
  }
  ...
  function nkScheduleCloudFlush(){clearTimeout(nkCloudTimer);nkCloudTimer=setTimeout(()=>nkCloudSync(false,true),1200);}
  ...
  function nkSyncPracticeCheckpoint(value){
    if(typeof nkNormalizeCheckpoint==='function')return nkNormalizeCheckpoint(value);
    if(!value||!value.sessionId||!Array.isArray(value.sessionQuestionIds)||!value.sessionQuestionIds.length)return null;
    return {...value,sessionId:String(value.sessionId),sessionQuestionIds:value.sessionQuestionIds.map(String),answers:{...(value.answers||{})},submitted:{...(value.submitted||{})},questionTimes:{...(value.questionTimes||{})},pendingFsrsRatings:{...(value.pendingFsrsRatings||{})},questionUpdates:{...(value.questionUpdates||{})}};
  }
  ...
  function nkMergePracticeCheckpoint(incoming){
    ...
    ids.forEach(id=>{
      ...
      const preferred=remoteNewer?remote:local,fallback=remoteNewer?local:remote;
    ...
    delete state.normalPracticeConflict;return merged;
  }
  ...
  async function nkPullCloud(token){
    let count=0;nkCloudMergeChanged=false;
    const apply=callback=>{
      const before=nkStable({state,activeSubject});nkCloudApplying=true;
      try{callback();}finally{nkCloudApplying=false;}
      if(before!==nkStable({state,activeSubject}))nkCloudMergeChanged=true;
    };
    for(const kind of NK_SYNC_KINDS){
      const docs=await nkPullKind(kind,token);
      apply(()=>docs.forEach(doc=>{nkApplyCloudEnvelope(doc);count++;}));
    }
  ...
  function nkCloudRenderStatus(){
    const status=document.querySelector?.('[data-nk-cloud-status]');if(status)status.textContent=nkCloudStatusCopy();
    const dot=document.querySelector?.('.nk-cloud-dot');if(dot)dot.className='nk-cloud-dot '+(nkSyncMeta.status==='error'?'is-error':navigator.onLine?'is-online':'');
    const detail=document.querySelector?.('.nk-cloud-error');if(detail){detail.hidden=!nkSyncMeta.lastError;const message=detail.querySelector('p');if(message)message.textContent=nkSyncMeta.lastError||'';}
  }
  ...
  async function nkCloudInit(){
    ...
    window.addEventListener('offline',()=>{nkSyncMeta.status='offline';nkSaveSyncMeta();nkCloudRenderStatus();});
