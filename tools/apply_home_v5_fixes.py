...
# Preserve the compatibility API and add one direct, deterministic subject route.
...
# Topics subject picker uses direct inline navigation and must not carry literal
# backslashes into the JavaScript handler (which would make the handler invalid).
...
new_picker = new_picker.replace(' data-subject="${encodeURIComponent(x.subject)}"', '')
new_picker = new_picker.replace('openSubjectTopics(\\\'${esc(x.subject)}\\\')', "openSubjectTopics('${esc(x.subject)}')")
...
# Final normalization for either representation generated by the replacement.
new_picker = new_picker.replace("openSubjectTopics(\\'${esc(x.subject)}\\')", "openSubjectTopics('${esc(x.subject)}')")
if 'openSubjectTopics' not in new_picker:
    raise SystemExit('Topics subject picker replacement did not produce direct route')
text = text[:start] + new_picker + text[end:]

# Scope the existing streak component to Home only.
...
# Every hash route transition begins at the top using a WebView-safe call.
...
# Normalize Home subject counts when the source stores arrays.
...
# Remove only the old global streak injector; the actual streak is now composed
...
print('Applied Home V7: subject buttons now directly open Topics, legacy modal interception bypassed, streak scoped to Home, route scroll reset retained, and NaN-safe counts retained.')