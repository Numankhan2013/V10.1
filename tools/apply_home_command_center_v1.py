...
HELPERS = r'''
  ...
  function nkMarkSkippedFromSession(s){
    if(!s?.questionIds?.length)return;state.fsrsReviewEligible=state.fsrsReviewEligible||{};s.questionIds.forEach(id=>{if(!s.answers?.[id])state.fsrsReviewEligible[String(id)]={reason:'skipped',at:Date.now()};});
  }
  const nkHomeOriginalEndSession=endSession;
  endSession=function(){nkMarkSkippedFromSession(state.activeSession);saveState();return nkHomeOriginalEndSession.apply(this,arguments);};
...
def transform(source: str) -> str:
    ...
    required=[FLOW_MARKER,'My Subjects','Strongest Chapters','Study Sessions',"Today's Review",'FSRS Review','wrong or encountered and skipped','Unseen QBank questions are never introduced','study-library',"['fsrs','FSRS'",'Full Question Bank','Custom Module','questions = ${nkTestSetup.count} minutes total',"timerMode='per-question'",'nkExpireTopicQuestion','Time expired','nkStartTopicTimedTest','nkOpenSubjectLibrary']
