...
HELPERS = r'''
  ...
  function nkFsrsReviewPage(){
    const subjects=['',...(SUBJECTS||[]).map(s=>s.subject)],selected=nkFsrsSubjectFilter,c=nkReviewCounts(selected),launch=nkFsrsLaunchQueue(selected),startCount=(launch.cards||[]).length,forecast=nkReviewForecast(selected),max=Math.max(1,...forecast);
    ...
    const rollover=launch.rolledOver?`<p class="nk-v3-page-note">${fmtNum(launch.rolledOver)} additional due review${launch.rolledOver===1?'':'s'} roll forward under your daily review limit.</p>`:'';
    return shell(`<main class="nk-app-v114 nk-fsrs-page-v3"><header class="nk-v3-page-hero"><div class="nk-kicker">SPACED REPETITION</div><h1>FSRS Review</h1><p>Every answered question is scheduled here. Unanswered questions enter only after you submit their session; pausing keeps untouched questions out.</p></header><section class="nk-v3-section"><div class="nk-v3-section-head"><h2>Choose subject</h2><span>${selected||'All subjects'}</span></div><div class="nk-fsrs-subject-list">${cards}</div></section><section class="nk-v3-section"><div class="nk-v3-section-head"><h2>Today's Review</h2><span>Review-only queue</span></div><div class="nk-review-metrics"><article><b>${fmtNum(c.due)}</b><small>Due</small></article><article><b>${fmtNum(c.learning+c.relearning)}</b><small>Learning</small></article><article><b>${fmtNum(c.overdue)}</b><small>Overdue</small></article></div><div class="nk-review-chart" aria-label="Seven day review forecast">${forecast.map((n,i)=>`<span><i style="height:${Math.max(6,Math.round(n/max*58))}px"></i><b>${n}</b><small>${i===0?'Today':'+'+i}</small></span>`).join('')}</div><button class="nk-v3-primary" onclick="window.QB.nkStartReviewOnly()">Start ${fmtNum(startCount)} due review${startCount===1?'':'s'} ${navIcon('chevron',17)}</button>${rollover}</section></main>`,'fsrs');
  }
...
TESTS_PAGE = r'''function testsPage(){
    nkTestSetup.mode='exam';
    const source=['wrong','bookmarks'].includes(nkTestSetup.source)?nkTestSetup.source:'all';
    const wrong=nkConfiguredQueueIds('wrong').length,bookmarks=nkConfiguredQueueIds('bookmarks').length;
    const available=source==='wrong'?wrong:source==='bookmarks'?bookmarks:0;
    const recent=(state.tests||[]).filter(t=>t.kind!=='practice').slice().sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
    return shell(`<main class="nk-new-test-v2 nk-tests-focused" aria-label="Timed tests"><header class="nk-v3-page-hero"><div class="nk-kicker">EXAM PRACTICE</div><h1>Timed tests</h1><p>Choose a question source, then work under exam conditions. Answers and explanations appear after submission.</p></header><section class="nk-v3-section nk-test-main-action"><div class="nk-v3-section-head"><h2>Build a test</h2></div><button class="nk-v3-primary" onclick="window.QB.openMultiSubjectTestBuilder()">Choose subjects and topics ${navIcon('chevron',17)}</button><p>Set the question count inside the test builder.</p></section><section class="nk-v3-section nk-test-quick-options"><div class="nk-v3-section-head"><div><small>FROM YOUR HISTORY</small><h2>Quick test</h2></div></div><div class="nk-test-source-list"><button class="nk-test-source-card ${source==='wrong'?'active':''}" ${wrong?'':'disabled'} onclick="window.QB.nkSetTestSource('wrong')"><span class="nk-test-source-icon is-wrong">${navIcon('refresh',23)}</span><span><strong>Wrong questions</strong><small>${fmtNum(wrong)} missed across all banks</small></span>${navIcon('chevron',20)}</button><button class="nk-test-source-card ${source==='bookmarks'?'active':''}" ${bookmarks?'':'disabled'} onclick="window.QB.nkSetTestSource('bookmarks')"><span class="nk-test-source-icon is-bookmarks">${navIcon('bookmark',23)}</span><span><strong>Bookmarked questions</strong><small>${fmtNum(bookmarks)} saved across all banks</small></span>${navIcon('chevron',20)}</button></div>${source==='all'?'':`<div class="nk-test-quick-setup"><h3>Number of questions</h3><div class="nk-test-count-grid">${[10,20,50,100].map(n=>`<button class="${nkTestSetup.count===n?'active':''}" onclick="window.QB.nkSetTestCount(${n})">${n}</button>`).join('')}</div><p>${fmtNum(Math.min(nkTestSetup.count,available))} questions available for this test.</p><button class="nk-test-start" onclick="window.QB.nkStartConfiguredTest()">Start timed test <span>→</span></button></div>`}</section><section class="nk-v3-section nk-test-history"><div class="nk-v3-section-head"><div><small>HISTORY</small><h2>Completed tests</h2></div><span>${fmtNum(recent.length)} total</span></div>${recent.length?`<div class="nk-v3-session-list">${recent.slice(0,5).map(testRow).join('')}</div>${recent.length>5?`<details class="nk-test-older"><summary>Show ${fmtNum(recent.length-5)} older tests</summary><div class="nk-v3-session-list">${recent.slice(5).map(testRow).join('')}</div></details>`:''}`:`<div class="nk-v3-empty">Your completed timed tests will appear here.</div>`}</section></main>`,'tests');
  }'''
...
/* NK UI polish 2026-09-23 — Home: same theme, cleaner depth (CSS only). Marker: nk-ui-polish-2026-09-23-home */
.nk-home-focus-card{background:linear-gradient(140deg,#4b41ab 0%,#38318c 55%,#2b266e 100%);box-shadow:0 18px 38px rgba(52,46,130,.30),inset 0 1px 0 rgba(255,255,255,.16)}
.nk-home-focus-card::before{content:"";position:absolute;inset:0;background:radial-gradient(440px 170px at 88% -20%,rgba(255,255,255,.17),transparent 62%);pointer-events:none}
.nk-home-focus-card h2{letter-spacing:-.4px}
.nk-home-focus-action{box-shadow:0 6px 16px rgba(18,14,58,.28);transition:transform .12s ease,box-shadow .12s ease}
.nk-home-focus-action:hover{transform:translateY(-1px);box-shadow:0 10px 24px rgba(18,14,58,.32)}
.nk-home-focus-action:active{transform:translateY(0);box-shadow:0 4px 12px rgba(18,14,58,.26)}
.nk-home-streak-card{box-shadow:0 8px 24px rgba(39,42,101,.07)}
.nk-home-streak-flame{box-shadow:inset 0 1px 0 rgba(255,255,255,.35),0 4px 10px rgba(255,140,50,.35)}
.nk-home-week-day.is-today i{outline:2px solid #7561ff;outline-offset:1px}
.nk-home-quick-grid button{transition:transform .12s ease,box-shadow .12s ease,border-color .12s ease}
.nk-home-quick-grid button:hover{transform:translateY(-1px);border-color:#cfd2e6;box-shadow:0 8px 20px rgba(39,42,101,.08)}
.nk-v3-subject-card{transition:transform .12s ease,box-shadow .12s ease,border-color .12s ease}
.nk-v3-subject-card:hover{transform:translateY(-1px);border-color:#cfd2e6;box-shadow:0 10px 24px rgba(39,42,101,.09)}
.nk-v3-subject-icon{box-shadow:inset 0 1px 0 rgba(255,255,255,.5)}
.nk-v3-subject-progress{height:6px}
...
.nk-tests-focused .nk-v3-page-hero{padding-bottom:4px}.nk-test-main-action{margin-top:12px}.nk-test-main-action .nk-v3-primary{margin-top:0}.nk-test-main-action>p,.nk-test-quick-setup>p{margin:8px 2px 0;color:#7b809d;font-size:10px}.nk-test-quick-options{margin-top:22px}.nk-test-source-card.active{border-color:#8c78ef;background:#f8f6ff}.nk-test-source-card:disabled{opacity:.55;cursor:not-allowed}.nk-test-quick-setup{margin-top:13px;padding:14px;border:1px solid #e6e7ef;border-radius:16px;background:#fff}.nk-test-quick-setup h3{margin:0;font-size:13px}.nk-test-quick-setup .nk-test-count-grid{margin-top:10px}.nk-test-history{padding-bottom:18px}.nk-test-older{margin-top:9px}.nk-test-older>summary{padding:12px;border:1px solid #e7e8f0;border-radius:12px;background:#fff;color:#6653bd;font-size:11px;font-weight:800;cursor:pointer}.nk-test-older .nk-v3-session-list{margin-top:8px}
</style>'''
...
def transform(source: str) -> str:
    ...
    dashboard = DASHBOARD
    dashboard = dashboard.replace('<button class="nk-home-search" aria-label="Open study library" onclick="window.QB.nav(\'study-library\')">${navIcon(\'search\',23)}</button>', '', 1)
    def remove_home_section(markup, marker):
        start = markup.find(marker)
        if start < 0:raise SystemExit(f'Home cleanup marker missing: {marker}')
        end = markup.find('</section>', start)
        if end < 0:raise SystemExit(f'Home cleanup end missing: {marker}')
        return markup[:start] + markup[end+len('</section>'):]
    for marker in ('<section class="nk-home-focus-card">', '<section class="nk-home-quick-grid"',
                   '<section class="nk-home-quote">', '<section class="nk-v3-section"><div class="nk-v3-section-head"><div><small>PERFORMANCE</small>',
...
def main() -> None:
    ...
    print('HOME_FLOW_V3_OK: hybrid Home, subject→Topics, FSRS capped review-only queue, global timed-test budget, strict topic timer')
