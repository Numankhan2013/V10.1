...
HELPERS = r'''
  ...
  function nkStartReviewOnly(subject=nkFsrsSubjectFilter){const queue=nkFsrsLaunchQueue(subject),rows=queue.cards||[];if(!rows.length){showToast('No wrong or skipped questions are due for this selection.');return;}BY_ID={...BY_ID,...Object.fromEntries(rows.map(q=>[String(q.id),q]))};startSession(rows.map(q=>String(q.id)),'practice','FSRS Review','fsrs');if(queue.rolledOver)showToast(`${queue.rolledOver} due reviews roll forward under your daily limit.`);}
...
def transform(source: str) -> str:
    ...
    required=[FLOW_MARKER,'My Subjects','Strongest Chapters','Study Sessions',"Today's Review",'FSRS Review','wrong or encountered and skipped','Unseen QBank questions are never introduced','study-library',"['fsrs','FSRS'",'Full Question Bank','Custom Module','questions = ${nkTestSetup.count} minutes total',"timerMode='per-question'",'nkExpireTopicQuestion','Time expired','nkStartTopicTimedTest','nkOpenSubjectLibrary','nkSessionQuestionEncountered','nkMarkSkippedFromSession(s)','nkFsrsLaunchQueue','nkFsrsQueue({subject})','due reviews roll forward under your daily limit']
