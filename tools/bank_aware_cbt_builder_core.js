  ...
  function nkCbtBuilderPage(){
    ...
    return shell(`<main class="nk-app-v114 nk-module-builder nk-cbt-builder ${step===2?'is-topics':''}" aria-label="Build timed test"><button class="nk-back-link" onclick="${step===1?"window.QB.nav('tests')":`window.QB.nkCbtSetStep(${step-1})`}">${navIcon('back',18)} ${step===1?'Tests':'Back'}</button>${nkAppPageHead(`TIMED CBT · STEP ${step} OF 3`,titles[step-1],step===2?'Search or scroll, then tap the topics you want to test.':'Build a test from the question banks you want to study.')}<div class="nk-module-stepper">${[1,2,3].map(n=>`<span class="${n===step?'is-current':n<step?'is-done':''}"><i>${n<step?navIcon('check',12):n}</i><b>${['Banks','Topics','Questions'][n-1]}</b></span>`).join('')}</div><section class="nk-module-builder-card">${content}</section>${action}</main>`,'tests');
  }
