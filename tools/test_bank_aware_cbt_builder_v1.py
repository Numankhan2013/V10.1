...
call('nkCbtSetStep(2)');
assert(call('nkCbtBuilderPage()').includes('1 topics · 1 Q'));
assert(call('nkCbtBuilderPage()').includes('nk-cbt-topic-tools'));
call('nkCbtSelectVerifiedPyqs()');assert.deepEqual(ids(),['prep1']);
assert.equal(call('nkCbtDraft.pyqSelected'),true);
call('nkCbtSelectVerifiedPyqs()');assert.deepEqual(ids(),[],'second tap removes selected PYQs');
assert.equal(call('nkCbtDraft.pyqSelected'),false);
call('nkCbtSelectVerifiedPyqs()');assert.deepEqual(ids(),['prep1']);
assert.equal(call('nkCbtTitle(nkCbtPool())'),'PYQ CBT');
call('nkCbtToggleTopic(0,1)');assert.deepEqual(ids().sort(),['prep1','prep2']);
assert.equal(call('nkCbtDraft.pyqSelected'),true,'regular topics can be added while PYQ toggle stays on');
call('nkCbtSelectVerifiedPyqs()');assert.deepEqual(ids(),['prep2'],'turning PYQ off preserves manually added regular topics');
call('nkCbtSelectVerifiedPyqs()');assert.deepEqual(ids(),['prep1'],'turning PYQ on replaces regular selection');
call('nkCbtToggleTopic(0,1)');
...
assert(call('nkCbtBuilderPage()').includes('No verified PYQ topics in the selected banks'));
