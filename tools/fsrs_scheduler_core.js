  ...
  function nkFsrsRatingMarkup(qid){
    ...
    return `<div class="nk-fsrs-rating" aria-label="Choose recall difficulty"><span>How difficult was recall?</span>${[2,3,4].map(r=>`<button type="button" class="${r===3?'is-default':''}" onclick="window.QB.nkRateCurrent(${r})"><b>${['','','Hard','Good','Easy'][r]}</b><small>${nkFsrsFormatInterval(new Date(previews[r].card.due)-Number(p.reviewedAt||Date.now()))}</small></button>`).join('')}</div>`;
  }
  ...
  function nkFsrsSetPreference(name,value){const p=nkFsrsPreferences(),n=Number(value),bounds={desiredRetention:[80,97],newCardLimit:[0,100],dailyCap:[20,300],maximumInterval:[30,3650]},b=bounds[name];if(!b)return;p[name]=name==='desiredRetention'?Math.min(b[1],Math.max(b[0],n))/100:Math.round(Math.min(b[1],Math.max(b[0],n)));saveState();render();}
  function nkFsrsSettingsMarkup(){const p=nkFsrsPreferences();return `<section class="nk-settings-group"><div class="nk-kicker">SMART REVIEW · FSRS 6</div><div class="card pad nk-fsrs-settings"><label>Desired retention <input type="number" min="80" max="97" value="${Math.round(p.desiredRetention*100)}" onchange="window.QB.nkFsrsSetPreference('desiredRetention',this.value)">%</label>${p.desiredRetention>.95?'<div class="nk-fsrs-warning">Above 95% can increase review workload sharply.</div>':''}<label>New cards / day <input type="number" min="0" max="100" value="${p.newCardLimit}" onchange="window.QB.nkFsrsSetPreference('newCardLimit',this.value)"></label><label>Total daily cap <input type="number" min="20" max="300" value="${p.dailyCap}" onchange="window.QB.nkFsrsSetPreference('dailyCap',this.value)"></label><label>Maximum interval (days) <input type="number" min="30" max="3650" value="${p.maximumInterval}" onchange="window.QB.nkFsrsSetPreference('maximumInterval',this.value)"></label><p class="small-muted">Changes apply to future ratings; existing due dates are not bulk-rescheduled.</p><button class="ghost-btn" onclick="window.QB.nkFsrsUndo()">Undo most recent rating</button></div></section>`;}
  ...
  const nkFsrsOriginalPractice=practicePage;practicePage=function(){const out=nkFsrsOriginalPractice(),s=state.activeSession,qid=s?.questionIds?.[s.index],markup=qid?nkFsrsRatingMarkup(qid):'';return markup?(out.includes('<div class="action-spacer"></div>')?out.replace('<div class="action-spacer"></div>',markup+'<div class="action-spacer"></div>'):out.replace('</section>',markup+'</section>')):out;};
