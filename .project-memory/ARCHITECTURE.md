...
- Public runtime config is generated by `write_runtime_config.py`; no privileged
  ...
  config only as fallback. Saves trigger debounced upload; signed-in clients also
  retry silently every five minutes and on foreground. Security ownership is enforced by `firestore.rules`.
