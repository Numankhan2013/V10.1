...
- Anatomy Ch7 Q11–Q21 is authored on `feature/marrow-explanations-anatomy-20260923-source-export-r2` as `candidate-rollout`, based on canonical `cb3f44b`; source-slice audit passed but the batch is not approved or packaged. Full build `35872090777` failed because wiring tried to package it. Candidate isolation is fixed locally; exact-head CI pending. Next: validate source/content, extend rollout regression, refresh inventory, run exact-head gates, then reconcile into current canonical.
- Inventory is **635 enhanced / 2,076 pending / 2,711 total** on this candidate base; the 11 candidate explanations are excluded. Raw source hashes are unchanged.
