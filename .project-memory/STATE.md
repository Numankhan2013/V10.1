...
- Latest verified CI checkpoint: `bf1be33`; Engineering `35451848708` and full Android/PWA/browser/APK/package run `35451848704` succeeded (live GitHub verified 2026-09-19). This includes the Biochemistry 4-3/5-10 stem repairs, the Biochemistry 5-14 option-B α-1,4 repair, and the new Biochemistry 4-12 explanation α-1,4 repair (all browser-verified: `BIOCHEM_4_3/5_10/5_14/4_12_BROWSER_OK`); user preview review is not blanket acceptance. Phase 3 audit re-run at `f5daabe` still reports OPEN with 219 source-review-required candidates (see Current priorities).
...
4. PrepLadder Biochemistry `4-3` (page-82 Q3 stem; page 83 A Hepatocytes / B Cardiac muscle / C RBC / D Neurons; answer `2`) and Biochemistry `5-10` (glycogen α-1,4 linkage, answer `3`) are repaired via stable-ID + source-fingerprinted `nkQuestionStemOverride`; Biochemistry `5-14` option B (page 102; PDF p123 solution + own explanation confirm α-1,4; answer `4`) is repaired via the new `nkQuestionOptionOverride` (raw/post-repair fingerprints `1528761351`/`2762270306`; in-memory display copy only, stored source unchanged). All three are build/browser-verified at the checkpoint above.
5. `python3 tools/test_question_presentation_v1.py` passes (8 option normalizations, 8 invalid, 58 semantic tables, 4-3/5-10 stem + 5-14 option + 4-12 explanation regressions). `python3 tools/verify_local.py` passes all 46 checks. `python3 tools/verify_project_memory.py` passes. Narrow generated-Practice browser regressions cover 4-3/5-10/5-14/4-12 (Linux CI only).
6. Phase 3 audit (`python3 tools/audit_question_structure.py`) re-run at `f5daabe`: 5,397 scanned; PrepLadder 444 candidates (36 complete-generic, 22 source-backed-override, 272 false-positive-ordinary-prose, 114 incomplete-unsafe, 8 fail-closed); Marrow 273 candidates (173 false-positive, 100 incomplete-unsafe, 0 fail-closed). Total source-review-required 219, unsafe-still-answerable 206, demonstrated rendered-cell defects still answerable 0. True literal-match/letter-mapping residuals without tables are narrow: PrepLadder `anatomy-9-1`, `anatomy-14-5`, `anatomy-40-10`, `anatomy-47-2` (all fail-closed image-dependent) plus combination-type `7-53`; Marrow 6 image-owned marked-structure matchings + row-selection/figure-point cases. Majority of incomplete-unsafe are structured-combination/row-selection/enumerated-list adjudication items, not proven parser loss (handoff: `.project-memory/PHASE3_STRUCTURE_ADJUDICATION_2026-09-19.md`). Phase 4 forensic complete (handoff: `.project-memory/PHASE4_NOTATION_FORENSIC_2026-09-19.md`): 60 questions with remaining ■ post-normalization; 5-14 repaired and build/browser-verified at the checkpoint above, awaiting user preview review. PrepLadder Biochemistry `4-12` explanation (PFK-1 regulator, answer `3`) had two glycogen-family ■-1,4 strings inside its over-long explanation tail: `connected by ■-1,4 linkage` (Q10 structure; PDF p117 solution confirms α-1,4) and `cleaves ■-1,4 linkages` (Q14 glycogenolysis; PDF p123 solution confirms α-1,4). Repaired via new stable-ID + explanation-fingerprinted `nkQuestionExplanationOverride` (7-field fingerprints raw `2733095777` / repaired `1814125949`; in-memory display copy only, stored source unchanged; exact-head unit `BIOCHEM_4_12_EXPLANATION_OK` + browser `BIOCHEM_4_12_BROWSER_OK` green). Built-app note: Practice renders Biochemistry explanations from source-PDF solution images, so this text repair serves Review/takeaway surfaces; the browser check asserts the PDF surface intact plus the live display copy repaired (first attempt failed by asserting text in the image surface; fixed in `bf1be33`). Known follow-up (not Phase 5 scope): 4-12's explanation tail embeds the full next-chapter glycogen dump (Q1-17 + answer table, ~13k chars) — notation fixed, content-trim needs source-review. Preserve `.project-memory/STATE.md.orig` untracked. Production promotion remains prohibited; user preview check is not blanket acceptance. Next: per-record Phase 5 repairs from authoritative pages only (ambiguous set: 14-9, physiology-9-18/20-7/23-11/26-20, Marrow CH14_Q013/CH17_Q008, F0/F1 + fragments). Phase 5 ambiguous adjudication complete with no further safe repairs (handoff: `.project-memory/PHASE5_AMBIGUOUS_ADJUDICATION_2026-09-19.md`): authoritative solution/source text layers carry the identical blocks (or are font-garbled for Marrow), the deterministic ■-1,4/■-1,6 set is empty, and diagram-owned 26-20 stays flagged; remaining notation residuals need rendered-page source review in a future lane.
