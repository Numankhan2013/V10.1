...
1. Integrate the verified multiple-paused-chapters behavior into `feature/home-polish-analysis-20260923`, then run exact-SHA Engineering, browser, APK and Android checks. Keep this branch separate; no canonical or production promotion.
2. Confirm the Home/Analysis UI polish still passes alongside multi-pause, including the 320px Home action and paused-session chooser. Physical device review remains outstanding.
3. Image automation remains independently owned from live canonical; P0 source review and sign-off remain separate.
