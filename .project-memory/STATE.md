...
- Exam practice: the user checked the earlier preview and confirmed the flow works, but found the PYQ action bulky and one-way. Product `af30703` puts a compact pressed-state toggle beside Select all/Clear all: on selects verified PYQ topics; off removes them while retaining manually added regular topics. Local 64 checks, Engineering `36233896258`, and full generated browser/PWA/APK/Android phone+tablet emulator `36233894250` passed. The browser journey verified 27 PrepLadder PYQ topics/1,118 questions, 10/410 Anatomy, 9/362 Physiology, 8/346 Biochemistry; toggle on/off and same-row layout, mixed CBT, saved history, Review Solutions, topic breakdown, follow-up Practice, and both-bank timer expiry. Browser and Android phone/tablet captures were visually inspected. Replacement preview `https://87a819e1.nk-qbank.pages.dev` returned HTTP 200. User review remains pending.
...
- Share the compact PYQ-toggle replacement preview. Then prioritize source-backed explanations in the current three subjects through a bounded batch on the exact canonical Marrow head. Tracker UI polish and new subject integration remain later work. Do not promote production or claim physical device acceptance from CI.
